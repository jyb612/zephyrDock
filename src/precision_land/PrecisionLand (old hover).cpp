...
{
	if (_search_started) {
		z_from_aruco = msg->pose.position.z;
		double x_offset = 0;  // Offset in meters for X
		double y_offset = 0.1;  // Offset in meters for Y

		auto tag = ArucoTag {
			.position = Eigen::Vector3d(msg->pose.position.x + x_offset, 
										msg->pose.position.y + y_offset, 
										msg->pose.position.z),
